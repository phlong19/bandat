...
function Header() {
  return (
    <header className="sticky left-0 top-0 flex h-24 w-full flex-wrap items-center justify-between bg-white px-4 py-[18px] text-zinc-800">
      <div className="flex items-center">
        <Link to="/" className="mt-2 h-12 w-40">
          <img src="" alt="logo" />
        </Link>
        <ul className="flex gap-5 pt-2.5">
          {navLinks.map((link, i) => (
            <li key={i} className="group inline-block pb-2.5">
              ...
              >
                {link.title}
                <div className="absolute mt-2.5 hidden w-64 flex-col gap-1 rounded bg-white px-2 py-3 pt-2 shadow-sm shadow-black group-hover:flex">
